...
```{r}        
fam <- read.table("02_activities/data/gwa.qc.A1.fam", header = FALSE)
nrow(fam)
...
printf "%s\n" rs1861, rs3813199, rs3128342, rs11804831 > ./02_activities/data/gwa.qc.A1.bim

```{r}        

bim <- read.table("02_activities/data/gwa.qc.A1.bim", header = FALSE)
nrow(bim)
...
plink2 \
  ...
  --freq counts \
  --out 02_activities/data/af_four_snps
...
plink2 \
  ...
  --snps rs1861,rs3813199,rs3128342,rs11804831 \
  --freq \
  --out 02_activities/data/maf_four_snps
...
plink2 --bfile C://Users//jasmine.kaur//shell//gen_data//02_activities//data//gwa.qc.A1 \
  --hardy \
  --out C://Users//jasmine.kaur//shell//gen_data//02_activities//data//hwe_all
...
```{r} 
hwe <- fread("./02_activities/data/hwe_all.hardy")
head(hwe)      
subset(hwe, ID %in% c("rs1861","rs3813199","rs3128342","rs11804831"))[, c("ID","P")]
```
...
plink2 \
  ...
  --snp rs1861 \
  --export A \
  --out ./02_activities/data/geno_rs1861
```

```{r}         

dat <- read.table("02_activities/data/geno_rs1861.raw", header = TRUE)
gcol <- grep("^rs1861", names(dat), value = TRUE)[1]
fit_add <- lm(PHENOTYPE ~ dat[[gcol]], data = dat)
summary(fit_add)$coefficients[2,4]
...
```         
The beta is the expected change in the phenotype per one additional copy of the coded allele at rs1861 (i.e., the slope of the additive genetic effect), holding other factors constant.
```
...
geno_add <- dat[[gcol]]
plot(geno_add, dat$PHENOTYPE,
     xlab = "rs1861 genotype (additive: 0/1/2)",
     ylab = "PHENOTYPE", pch = 19, col = "#1f77b4")
abline(lm(dat$PHENOTYPE ~ geno_add), col = "red", lwd = 2)
...
```{r}      
geno_rec <- ifelse(is.na(geno_add), NA, ifelse(geno_add == 2, 1, 0))
```
...
fit_rec <- lm(dat$PHENOTYPE ~ geno_rec)
summary(fit_rec)$coefficients[2,4]
...
plot(geno_rec, dat$PHENOTYPE,
     xlab = "rs1861 recessive genotype (0/1)",
     ylab = "Phenotype", pch = 19, col = "#2ca02c", xaxt = "n")
axis(side = 1, at = c(0,1), labels = c("Non-homozygote","Homozygote coded"))
abline(lm(dat$PHENOTYPE ~ geno_rec), col = "red", lwd = 2)
...
c(AIC_additive = AIC(fit_add), AIC_recessive = AIC(fit_rec))
# The model with the lower AIC (or higher adjusted R^2) fits better.
